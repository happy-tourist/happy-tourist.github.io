...
| `work-with-game-board`        | Board + top opponents / spectator presence + seated strip (row/2×2; no chip/`q-menu`) + grille (`GRILLE_ANIM_MS=1000`) + trap/rescue/push top-center + return strip icon (no modal) + budgets / end-turn `skip_next` on avatar + dual rings + say top↓/own↑ + push/move finish travel (`lastKnown` seed; finish watch owns `finishAnimFrom`) + return anim |
