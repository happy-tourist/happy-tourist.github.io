...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / `listMyModeration` / live (`inCollection`, `inCatalog`) / working-copy draft (`/draft` = creator unpublished) / unified `submitPack` / add-task-set / staff lock + `staffSavePack` / `isStaffEditSessionNavigation` (cards↔tasks keep lock) / `unpublishPack` / `republishPack` / `needsRevisionRequest` / `cancelRequest` / author delete unpublished; open = pending\|needs_revision; cascade yellow helpers; no personal drafts / stale rebase; `loading` / `error` + page `q-banner` (quiet autosave; `pack_unpublished`).
...
| `client-locate-change-points` | Where to edit/add (incl. content working copy / add-task-set beside «Задания» / staff lock session + cascade yellow / trash/`inCollection` / «На модерации» hidden for staff / support `change_pack`; Vitest harness paths)                                                                                                                                                                                                                                                                                                                                  |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content working copy + add-task-set beside «Задания» + staff Edit lock session + trash/click isolation / author «На модерации» / cascade yellow / author delete unpublished / staff needs_revision / no block UI)                                                                                                                                                                                                                                                     |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage / Support create (`change_pack` pack select) `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации» (hidden for staff); content working-copy editor + add-task-set beside «Задания» + staff Edit lock session + stable tooltip hints; trash/click isolation; cascade yellow; staff needs_revision; support `change_pack`; no block UI; GamePage HUD / board                                                                                                                                                                                                                                  |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` (`change_pack`+packId) / `content` (working copy + `submitPack` + add-task-set + staff lock/save + `isStaffEditSessionNavigation` + cascadeGap* + needs_revision / `inCollection` / no block UI; meta topic `work-with-stores/content.md`)                                                                                                                                                                                                                                                                       |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` («Набор карточек» / `addTaskSet*` / `staffEditSubtitle` / `tasksSaveHint` / `questionNeedsSlot` / `deleteCardConfirm*` by hasLive / `myModeration*` / `statuses.needs_revision` / errors); game keys                                                                                                                                                                                                                                                                                   |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL SC-PACK-100…119 + SupportChangePack; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                                                                             |
