...
| `client-locate-change-points` | Where to edit/add (incl. content working copy / add-task-set beside «Задания» / staff lock session + cascade yellow + pack/set soft-unpublish + live drill-in SC-PACK-126…133 / trash/`inCollection` / «На модерации» hidden for staff / support `change_pack`; Vitest harness paths)                                                                                                                                                                                                                                                                                                                |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content working copy + add-task-set beside «Задания» + staff Edit lock session + trash/click isolation / author «На модерации» / cascade yellow + pack/set soft-unpublish + live drill-in SC-PACK-126…133 / author delete unpublished / staff needs_revision / no block UI)                                                                                                                                                                                                                                   |
...
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации» (hidden for staff); content working-copy editor + add-task-set beside «Задания» + staff Edit lock session + stable tooltip hints; trash/click isolation; cascade yellow + slot chips + add-task-set thread + pack/set soft-unpublish + live drill-in + AddTaskSet chips (SC-PACK-126…133); staff needs_revision; support `change_pack`; no block UI; GamePage HUD / board                                                                                                                                                                             |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` (`change_pack`+packId) / `content` (working copy + `submitPack` + add-task-set + staff lock/save + `isStaffEditSessionNavigation` + cascadeGap* + pack/set soft-unpublish + live drill-in SC-PACK-126…133 / `inCollection` / no block UI; meta topic `work-with-stores/content.md`)                                                                                                                                                                                                                                                                      |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` («Набор карточек» / `addTaskSet*` / `staffEditSubtitle` / `unpublishConfirm*` / `unpublishTaskSetConfirm*` / `taskSetDifficultySummary` / `lastPublishedTaskSetHint` / `taskSetStatusMarks.*` / `moderationThread` / `slotEmpty` / `deleteCardConfirm*` by hasLive / `myModeration*` / `statuses.needs_revision` / errors incl. `last_published_task_set`); game keys                                                                                                                                                                                                                       |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL SC-PACK-100…133 + `ContentSoftUnpublish` + `ContentFollowUp4` + `ContentModerationUx` + SupportChangePack; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                            |
