...
- Open Game and view the tourist board with synced seats (pieces appear only in `playing`) and a personal strip×4 inside the sticky bottom HUD once own pieces exist (no «Мои туристы» caption); after phase `playing`, on own turn select an unfinished piece and submit a one-step `move` via the game store; landing on center finishes a piece (disappear + strip icon); finishing all four shows a place modal and presence badge while the seat stays (say allowed, no moves, leave without confirm); solo five-minute timer expiry or steps exhaustion (no live task tile) shows a distinct end modal + locks moves (`timeExpired`); presence lives in the sticky bottom `.game-hud` (seated: own+budgets → strip → opponents right; spectator: occupied centered; no top-row / side columns) with dual circular countdowns (outer turn blue/red, inner reconnect warning) around a strip-sized avatar; say bubbles stack above avatars toward the board; seated+online players may send preset say bubbles (`hello` / `luck`) via `sendSay`; underfilled waiting may `sendReady`.
...
| `work-with-pages`             | Routes + guards; App header (theme + Game leave/status); Lobby grilleDensity; GamePage sticky `.game-hud` / budgets/peek / holes / grilles / dual end + all-jail |
...
| `work-with-styles`            | Quasar Dark + GET/POST `/api/theme`, header, muted chrome, board holes + grille overlays (`--grille-anim-ms` 1500) + sticky `.game-hud` / budgets/peek CSS      |
...
| `work-with-game-board`        | Board + sticky bottom `.game-hud` presence + grille overlay (`GRILLE_ANIM_MS=1500`) + trap/rescue/return + budgets/end-turn + peek + holes + dual rings + strip  |
...
- [`../happy-tourist-server`](../happy-tourist-server) — Colyseus multiplayer server (rooms, auth, HTTP `/rooms/:roomName`). Prefer changing room names, state schema, and move protocol in coordination with the server; this client assumes room type `tourist`, mirrors seats (`touristId` + `pieces` + `finished`/`trapped` / `finishPlace` / `timeExpired`) / `phase` / `maxSeats` / `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys` / `holdingGrilleKeys`, listens private `budgets`/`peekOpen`/`allJailWarning`, renders unfinished pieces only after materialize + holes (not landable) + grille overlays + trap/rescue/return chrome + sticky bottom `.game-hud` presence with dual rings + own counters (steps number / peeks ∞ solo) /end-turn + peek chrome + strip finish/return + place/timer-vs-steps/solo-peeks∞/all-jail modals + local move chrome only in `playing` for eligible seats (move does not end turn; targets exclude holes; trapped locked), and sends via `sendMove` / `sendRescue` / `sendReturnFromFinish` / `sendPeek` / `sendPeekAnswer` / `sendEndTurn` / `sendReady`. Create options `{ maxSeats, grilleDensity }`. Leave + match status live in `App.vue` header on Game (no room-id chrome).
