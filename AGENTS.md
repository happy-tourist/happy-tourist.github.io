...
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации» (hidden for staff); content working-copy editor + add-task-set beside «Задания» + staff Edit lock session + stable tooltip hints; trash/click isolation; cascade yellow + slot chips + add-task-set thread + pack/set soft-unpublish + live drill-in + AddTaskSet chips + taskSetLabelFrom / Tasks `back` (SC-PACK-126…136); staff needs_revision; support `change_pack`; no block UI; GamePage HUD / board                                                                                                                |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` («Набор карточек» / `addTaskSet*` / `staffEditSubtitle` / `unpublishConfirm*` / `unpublishTaskSetConfirm*` / `taskSetDifficultySummary` / `taskSetLabelFrom` / `back` / `lastPublishedTaskSetHint` / `taskSetStatusMarks.*` / `moderationThread` / `slotEmpty` / `deleteCardConfirm*` by hasLive / `myModeration*` / `statuses.needs_revision` / errors incl. `last_published_task_set`); game keys                                                                                                                  |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL SC-PACK-100…136 + `ContentSoftUnpublish` + `ContentFollowUp4` + `ContentFollowUp5` + `ContentModerationUx` + SupportChangePack; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                       |
