...
- Browse content packs: collection → catalog/create; answers page + nested task-set edit; dual submit answers/tasks (non-anonymous + `emailVerified`); staff answers-pending hub (moderator|admin).
...
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create; answers editor (`…/edit`) + nested task-set (`…/tasks/:taskSetId`); dual submit; staff answers-pending hub. HTTP only via `client.http` (no Colyseus room). Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified` (ineligible → login/verify modal); dirty answers lock tasks (no badge); staff `requiresStaff`; approve order tasks → answers → catalog. Live = approved snapshot; blocked listed with badge. Capability: `openspec/specs/content/packs/`.
...
| `/content/packs/:id/edit`             | `content-pack-edit`       | answers page (title/cards + task-set list); `meta.requiresAuth`         |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`      | nested task-set editor; `meta.requiresAuth`                             |
| `/content/packs/:id/moderation`       | `content-pack-moderation` | author moderation thread (type-scoped); `meta.requiresAuth`             |
| `/content/staff`                      | `content-staff`           | answers-pending queue; `meta.requiresAuth` + `requiresStaff`            |
| `/content/staff/requests/:id`         | `content-staff-request`   | answers hub + nested tasks; `requiresStaff`                             |
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / live / draft / `submitAnswers` / `submitTasks` / moderation + staff; `answersDirty` + pending flags per type (no badge state); `loading` / `error` + page `q-banner`.
...
| `client-work-with-structure`  | pages / components / boot / stores / assets (`brand/`, `grilles/`, `catapults/`) placement (always-button brand logo ≥60px shell + Game leave via logo + status in App; content answers + nested tasks + dual submit) |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `work-with-pages`             | Routes + guards login/forgot/confirm/reset/account/lobby/support/content/admin/game; App header (always-button brand logo ≥60px + theme + verify reminder + Game leave via logo/status; auth→lobby; lobby noop; no page «В лобби»; productName/favicon); Lobby grilleDensity+catapultDensity + Support + content («Наборы» → collection); content answers + nested tasks + staff answers hub; GamePage top presence + seated strip HUD / budgets / end-turn icon / return strip icon / push / holes / grilles / catapult land→overlay→fling + deferred grille + board-busy / dual end + all-jail |
| `work-with-stores`            | Pinia `auth` (forgot/confirm/reset/change-email / `role` gating) / `theme` / `game` (incl. peeks∞ / finite steps / peek / rescue/push/return / grilleDensity+catapultDensity create / reveal keys / D13 atomic `$patch` seats+revealing / end-turn / `consentedLeaving`) / `support` (HTTP tickets + staff + admin) / `content` (dual submit answers/tasks + `answersDirty` / pending per type; catalog/collection/draft/staff)                                                                                                                                                                  |
