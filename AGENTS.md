...
- Browse content packs: collection → catalog/create; author «На модерации» (`/content/my-moderation`); single creator working copy until first catalog approve; unified Submit; collection Edit (unpublished creator or staff) + trash (click isolation; hide Edit if `blocked`); published non-staff → add-task-set only; live staff Edit (exclusive lock) / non-staff add-task-set; staff queue pending|needs_revision (Approve / «Доработать»); author delete unpublished; cascade yellow; support topic `change_pack` + catalog pack select; no block UI.
...
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create + author «На модерации» (`content-my-moderation`). **Single creator working copy** until first catalog approve; no personal drafts / dual submit / unpublish-republish / draftStale. Collection: Edit (unpublished creator **or** staff any pack) + trash confirm; published non-staff → «Добавить набор заданий» only; **no** row `:to` — row `@click` → live if `hasLive` else editor; icon `@click.stop`; hide Edit if `blocked`. Live: staff Edit (exclusive lock) without collection; non-staff verified collectors → add-task-set page (no editable lists of existing cards/sets). Creator editor: unified Submit (cards + ≥1 task set); staff editor: direct `staffSavePack` (no Submit). Staff queue: open `pending`|`needs_revision`; single Approve / «Доработать» (`needs-revision`); author Cancel. Cascade yellow / D1′ locks retained on unpublished working copy. HTTP via `client.http`. Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified`; staff `requiresStaff`. Capability: `openspec/specs/content/packs/`.
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / `listMyModeration` / live (`inCollection`) / working-copy draft (`/draft` = creator unpublished) / unified `submitPack` / add-task-set / staff lock + `staffSavePack` / `isStaffEditSessionNavigation` (cards↔tasks keep lock) / `needsRevisionRequest` / `cancelRequest` / author delete unpublished; open = pending\|needs_revision; cascade yellow helpers; no personal drafts / unpublish / stale rebase; `loading` / `error` + page `q-banner` (quiet autosave).
