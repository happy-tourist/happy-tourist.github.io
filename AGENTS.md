...
- Browse content packs: collection → catalog/create; answers page («Набор карточек») + nested task-set edit; dual submit answers/tasks (non-anonymous + `emailVerified`); statuses + embedded moderation threads; staff answers-pending hub → task-set list → nested tasks (moderator|admin).
...
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create; answers editor (`…/edit`, copy «Набор карточек») + nested task-set (`…/tasks/:taskSetId`); dual submit; quiet autosave (no top «Сохранение…»; submit disabled without dirty/minima). Draft statuses answers/tasks (`pending`|`rejected`|`approved`|none) + requestId; embedded threads + reply; reject comment visible; per-set `needsModeration`; `answersDirty` / `tasksDirty`. D5′ lock: only answers-pending author resubmits answers; others cannot submit tasks while answers pending. Staff hub = task-set list → nested tasks (approve/reject answers on hub, tasks on nested). HTTP only via `client.http` (no Colyseus room). Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified` (ineligible → login/verify modal); staff `requiresStaff`; approve order tasks → answers → catalog. Live = approved snapshot; blocked listed with badge. Capability: `openspec/specs/content/packs/`.
...
| `/content/collection`                 | `content-collection`      | own collection (lobby entry); `meta.requiresAuth`                                        |
...
| `/content/packs/:id/edit`             | `content-pack-edit`       | «Набор карточек» (title/cards + statuses/thread + task-set list); `meta.requiresAuth`    |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`      | nested task-set (statuses/thread; lock if dirty/pending); `meta.requiresAuth`            |
...
| `/content/staff/requests/:id`         | `content-staff-request`   | answers hub (task-set list) → nested tasks; `requiresStaff`                              |
| `/content/staff/requests/:id/tasks`   | `content-staff-request-tasks` | nested tasks review + approve/reject/cancel tasks; `requiresStaff`                    |
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / live / draft / `submitAnswers` / `submitTasks` / moderation + staff; draft statuses + requestId; `answersDirty` / `tasksDirty` + per-set `needsModeration` + pending flags per type (no badge state); D5′ pending-author lock mirrored in UI; `loading` / `error` + page `q-banner` (quiet autosave — no top save banner).
...
| `client-work-with-structure`  | pages / components / boot / stores / assets (`brand/`, `grilles/`, `catapults/`) placement (always-button brand logo ≥60px shell + Game leave via logo + status in App; content «Набор карточек» + nested tasks + dual submit / statuses / threads / staff nested)                                                                                                                                                                                                                                                                                                                                                |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `work-with-pages`             | Routes + guards login/forgot/confirm/reset/account/lobby/support/content/admin/game; App header (always-button brand logo ≥60px + theme + verify reminder + Game leave via logo/status; auth→lobby; lobby noop; no page «В лобби»; productName/favicon); Lobby grilleDensity+catapultDensity + Support + content («Наборы» → collection); content «Набор карточек» + statuses/threads + staff task-set nested; GamePage top presence + seated strip HUD / budgets / end-turn icon / return strip icon / push / holes / grilles / catapult land→overlay→fling + deferred grille + board-busy / dual end + all-jail |
| `work-with-stores`            | Pinia `auth` (forgot/confirm/reset/change-email / `role` gating) / `theme` / `game` (incl. peeks∞ / finite steps / peek / rescue/push/return / grilleDensity+catapultDensity create / reveal keys / D13 atomic `$patch` seats+revealing / end-turn / `consentedLeaving`) / `support` (HTTP tickets + staff + admin) / `content` (dual submit + `answersDirty`/`tasksDirty` / statuses / D5′ lock / staff nested)                                                                                                                                                                                                  |
