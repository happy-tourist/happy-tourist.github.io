...
- Browse content packs: collection → catalog/create; collection list Edit + trash (`delete`) + confirm remove (click isolation; hide Edit if `blocked`); live Edit when `inCollection` (hide if `blocked`/foreign pending; pending author keeps Edit; collect from `inCollection`); answers page («Набор карточек») + nested task-set; dual submit; author delete unpublished pack/task-set (confirm); D1′ task lock; statuses + threads; staff queue answers + **tasks-only**; same hub layout; no answers approve when tasks-only; nested approve → hub (or queue if tasks-only) (no block UI).
...
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create. Collection: Edit icon + trash (`delete`) + confirm remove; **no** row `:to` (Quasar race) — row `@click` → live if `hasLive` else editor; icon clicks MUST `@click.stop`; hide Edit if `blocked`. Live pack: Edit when `inCollection` and (no pending or I am pending author); hide Edit if not in collection, `blocked`, or foreign pending; collect button from live `inCollection` (not ephemeral `added`). Live GET exposes `inCollection` + `pendingAnswersAuthorId` / `pendingTasksAuthorId`. Answers editor (`…/edit`, «Набор карточек») + nested task-set (`…/tasks/:taskSetId`); dual submit; quiet autosave; author **delete unpublished** pack / task-set (creator only, confirm). Draft statuses + requestId; threads; per-set `needsModeration`; `answersDirty` / `tasksDirty`. **D1′:** dirty answers without pending → lock tasks for all; answers-pending author A MAY create/edit tasks while dirty; others blocked. D5′: only answers-pending author resubmits answers; others cannot submit tasks while answers pending. Staff queue lists answers-pending **and** tasks-only (one row per pack). Same hub layout (answers context → task-set list); `tasksOnly` / `answersActionsAvailable === false` → hide answers approve/reject; nested tasks → redirect hub after approve, or **queue** when tasks-only (omit moderated sets; no `pack_not_public` dead-end; **no block/unblock UI**). HTTP via `client.http`. Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified`; staff `requiresStaff`; approve order tasks → answers → catalog. Capability: `openspec/specs/content/packs/`.
...
| `/content/packs`                      | `content-catalog`             | pack catalog (link from collection); `meta.requiresAuth`                                                             |
| `/content/collection`                 | `content-collection`          | own collection (lobby entry; Edit + trash + confirm; no row `:to`; hide Edit if `blocked`); `meta.requiresAuth`      |
...
| `/content/packs/:id/edit`             | `content-pack-edit`           | «Набор карточек» (+ author delete unpublished); `meta.requiresAuth`                                                  |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (D1′ lock; author delete set); `meta.requiresAuth`                                                   |
...
| `/content/staff`                      | `content-staff`               | answers + tasks-only queue; `meta.requiresAuth` + `requiresStaff`                                                    |
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection (confirm remove) / live (`inCollection` + `pendingAnswersAuthorId`/`pendingTasksAuthorId`; patch membership on collect/remove) / draft / `submitAnswers` / `submitTasks` / author delete unpublished pack|task-set / moderation + staff; draft statuses + requestId; `answersDirty` / `tasksDirty` + per-set `needsModeration` + pending per type; D1′ + D5′ locks mirrored in UI; staff pending/`tasksOnly`/`answersActionsAvailable`; hub omit moderated sets / post-approve → hub (queue if tasks-only); no block UI; `loading` / `error` + page `q-banner` (quiet autosave).
...
| `client-locate-change-points` | Where to edit/add (incl. content packs D1′/author delete / live Edit in-collection + trash / `inCollection` / staff queue tasks-only + hub `tasksOnly`; Vitest harness paths)                                                                                                                                                                                                                                                                                                                                                                              |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content live Edit in-collection + trash/click isolation (no row `:to`; hide Edit if `blocked`) / author delete unpublished / D1′ / staff queue tasks-only + hub `tasksOnly` / no block UI)                                                                                                                                                                                                                                                                             |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; content trash + click isolation (no row `:to`; hide Edit if `blocked`); live Edit when `inCollection` (+ pending-author; hide if blocked/foreign pending); author delete unpublished; D1′ lock; staff queue tasks-only + same hub / no answers approve when tasks-only; no block UI; GamePage HUD / board                                                                                                                                                                                                      |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` / `content` (dual submit + D1′/D5′ locks / live `inCollection` + `pendingAnswersAuthorId`/`pendingTasksAuthorId` / collection trash confirm / author delete unpublished / staff `tasksOnly`/`answersActionsAvailable` / no block UI)                                                                                                                                                                                                                                                                             |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `content.*` («Набор карточек» / trash remove confirm / D1′ lock / staff queue+tasks-only hub hint + nested + errors); game keys                                                                                                                                                                                                                                                                                                                                                                                                     |
