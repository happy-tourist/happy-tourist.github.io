...
- Browse content packs: collection → catalog/create; author «На модерации» (`/content/my-moderation`); collection list Edit + trash (`delete`) + confirm remove (click isolation; hide Edit if `blocked`); live Edit when `inCollection` (hide if `blocked`/foreign open; open author keeps Edit; collect from `inCollection`); answers page («Набор карточек») + nested task-set; dual submit; author delete unpublished pack/task-set (confirm); D1′/D5′ on open (pending|rejected); three-phase marks (ожидает отправки / на модерации / нужна доработка); statuses + threads; staff queue answers + **tasks-only** + open **rejected** («нужна доработка»; approve-from-rejected OK); same hub layout; no answers approve when tasks-only; nested approve → hub (or queue if tasks-only) (no block UI).
...
| `/content/packs/:id/edit`             | `content-pack-edit`           | «Набор карточек» (+ three-phase marks; author delete unpublished); `meta.requiresAuth`                               |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (D1′ lock; three-phase marks; author delete set); `meta.requiresAuth`                                |
...
| `client-locate-change-points` | Where to edit/add (incl. content packs D1′/author delete / live Edit in-collection + trash / `inCollection` / author «На модерации» / staff queue reject-stays + hub `tasksOnly`; Vitest harness paths)                                                                                                                                                                                                                                                                                                                                                      |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content live Edit in-collection + trash/click isolation / author «На модерации» / three-phase marks / author delete unpublished / D1′ / staff reject-stays + approve-from-rejected / hub `tasksOnly` / no block UI)                                                                                                                                                                                                                                                   |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации»; content trash + click isolation; live Edit when `inCollection`; three-phase marks; staff queue rejected + approve-from-rejected; tasks-only hub; no block UI; GamePage HUD / board                                                                                                                                                                                                                                                                                                      |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` / `content` (dual submit + my-moderation / D1′/D5′ / `inCollection` + pending author ids / three-phase / staff pending\|rejected + approve-from-rejected / `tasksOnly` / no block UI)                                                                                                                                                                                                                                                                                                                            |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `content.*` («Набор карточек» / `myModeration*` / `statusCycle*` three-phase / staff rejected label + nested + errors); game keys                                                                                                                                                                                                                                                                                                                                                                                                   |
