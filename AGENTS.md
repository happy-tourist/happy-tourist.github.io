...
- **Game session** - `stores/game` room attach (mirror `seats` with `touristId` + `pieces[]` (+ `finished`) + `connected` / `reconnectUntil` / `ready` / `finishPlace` / `timeExpired` / `phase` / `maxSeats` / `countdownRemaining` / `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys` / `sessionId`; private `steps`/`peeks`/`budgetsInfinite`/`peekedThisTurn`/`openPeek` from `budgets`/`peekOpen`; `isMyTurn` / `isPlaying` / `isMySeatFinished` / `isMySeatTimeExpired` / `canSendEndTurn` / `sendMove` / `sendPeek` / `sendPeekAnswer` / `sendEndTurn` / `sendReady`) + `pages/GamePage` tourist board (unfinished pieces + disappear + removed-task holes) + presence rows + dual rings + own counters + end-turn + corner affordances + strip×4 once pieces exist + countdown overlay + leave confirm when seated ∧ `playing` ∧ `!finishPlace` ∧ `!timeExpired`.
- **Tourist board** - client layout constant on GamePage (start/task/center tiles; holes for `removedTaskKeys`); overlay unfinished pieces only after materialize; occupied presence in top/bottom rows (no side columns) with outer turn countdown (blue 60s / red solo 300s) + inner reconnect warning, reserved 96px chrome around 72px avatar (no static turn outline); finish/ready top-left, say top-right; own steps/peeks + «Завершить ход»; peek eye (store `peekedThisTurn`) + Correct/Wrong modal; keep-focus after non-finishing move; +N budget fall ≈ 2 s; bubbles toward board; header «Ваш ход» / «Ход соперника» / «Ход игрока»; countdown overlay; on own turn while `playing` local select/hints + `sendMove` (does not end turn); center finish → fade + strip icon + place modal; solo timeout / ∞ modals.
...
| `work-with-pages`             | Routes + guards; App theme header; GamePage presence/budgets/peek overview                                  |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` (incl. budgets / peek / end-turn)                                           |
| `work-with-styles`            | Quasar Dark + GET/POST `/api/theme`, header, muted chrome, board + presence/budgets/peek CSS                |
| `work-with-localization`      | vue-i18n boot; `game.say` / ready / leave / finish / timeout / steps/peeks / endTurn / peek / solo-∞ keys  |
...
| `work-with-game-board`        | Board + presence + budgets/end-turn + peek eye/modals + holes + dual rings + strip + move/say               |
...
- [`../happy-tourist-server`](../happy-tourist-server) — Colyseus multiplayer server (rooms, auth, HTTP `/rooms/:roomName`). Prefer changing room names, state schema, and move protocol in coordination with the server; this client assumes room type `tourist`, mirrors seats (`touristId` + `pieces` + `finished` / `finishPlace` / `timeExpired`) / `phase` / `maxSeats` / `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys`, listens private `budgets`/`peekOpen`, renders unfinished pieces only after materialize + holes + presence rows (top/bottom) with dual rings + own counters/end-turn + peek chrome + strip finish + place/timeout/solo modals + local move chrome only in `playing` for eligible seats (move does not end turn), and sends via `sendMove` / `sendPeek` / `sendPeekAnswer` / `sendEndTurn` / `sendReady`.
