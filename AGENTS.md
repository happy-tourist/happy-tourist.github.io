...
- Browse available tourist rooms in the lobby (live `LobbyRoom` subscribe; leave lobby before enter `tourist`); open Support, content packs («Наборы» → collection first), or content maps («Карты») from the lobby header.
- Create support tickets (topic + body; `change_pack` + catalog pack select), view own list / thread; staff (moderator|admin) queue; admin users page to change roles.
- Browse content packs: collection → catalog/create; author «На модерации» (`/content/my-moderation`); single creator working copy until first catalog approve; unified Submit; collection Edit (unpublished creator or staff) + trash (click isolation; hide Edit if `blocked`; soft-unpublished gray for non-staff); published non-staff → add-task-set only (in-catalog; status + moderation thread while open request); live staff Edit (exclusive lock) / soft-unpublish/republish (pack confirm warns open requests cancelled — SC-PACK-139) / non-staff add-task-set; question lists show answer slot chips; staff queue pending|needs_revision (Approve / «Доработать»); author delete unpublished; cascade yellow (`cascade-gap-outline` on Editor + Tasks); support topic `change_pack` + in-catalog pack select; no block UI.
- Browse content maps («Карты» `/content/maps`): approved in-catalog for any JWT; author’s own never-published drafts; paint editor 10×10 + players×tourists; submit / my-moderation / shared staff queue type badge pack|map; soft-unpublish + staff Edit lock; no collection; **wired into tourist-room create** (map picker + snapshot).
- Create a game with chosen **map** + **pack/task sets** + grille/catapult density (capacity = map.players, including 1), or join by room id.
...
- Shared elevated `q-header` in `App.vue` (always-button brand logo ≥60px left; Packs/Maps/Support + staff Модерация; wide Acc/Theme/Logout; narrow section burger rightmost with Acc/Theme/Logout in menu — SC-BRAND-19; on Game centered match status + leave = logo click and `header-game-leave`). Packs/maps/moderation breadcrumbs sit **inside** `q-page-container` (not in the elevated header — SC-BRAND-17…18). Guest preference → `localStorage` (`ht-theme`); registered → `GET /api/theme` restore (not JWT-only) + `POST /api/theme` on toggle. Tourist board scoped CSS is independent of chrome Dark.
...
- Scripts: `dev` (`quasar dev`), `build` (`quasar build`), `lint` / `lint:check`, `typecheck`, `test` (`vitest run`), `test:watch` (`vitest`).
- Unit tests: Vitest + `@vue/test-utils` + `jsdom` (`vitest.config.ts`, `test/setup.ts` — Pinia/`$t`/`useI18n` + mocked `@/boot/colyseus`). Conventions: meta skill [`work-with-test`](../happy-tourist-meta/.agents/skills/client/work-with-test/SKILL.md). OpenSpec client tasks include vitest + `npm test` (not lint/typecheck alone). Not mocha/Jest (server uses mocha).
...
6. Root `App.vue` mounts `q-layout` → shared `q-header` (always-button brand logo ≥60px left; theme toggle; on Game also status + leave via logo; auth/other → lobby, lobby noop) → `router-view`; stable `watch` on `auth.ready` / user id / anonymous restores theme via `GET /api/theme` (registered) or `localStorage` (guest) — do not replace `auth.user` after GET (avoids restore request storm).
...
- `q-layout` → `q-header` (always-button brand logo ≥60px left; theme toggle; on Game centered match status; leave via logo click) → `q-page-container` → theme `q-banner` + `<router-view />`.
- On Game: leave confirm `q-dialog` + `leaveGame` → lobby (`stores/game`); auth logo → lobby (guest may bounce via guard); lobby logo noop; other authenticated logo → lobby (no page «В лобби»).
...
- **Support** - `stores/support` + `pages/SupportPage` (create + own list; topic `change_pack` + catalog pack select title/description) + `SupportTicketPage` (thread; packId + live link in body for change_pack) + `SupportStaffPage` (staff queue; moderator|admin; topic + status filters, default all topics / open) + lobby header link «Поддержка» (any JWT, incl. anonymous). HTTP only via `client.http` (no Colyseus room). Guest create: warn no email notify + session-loss risk; anonymous display «Гость»; create/reply: `lazy-rules` + clear → `await nextTick()` → `resetValidation` (no red empty after success); thread message spacing + gap thread→reply; closed read-only + CTA new ticket. i18n RU for topics/statuses/limits.
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create + author «На модерации» (`content-my-moderation`). **Single creator working copy** until first catalog approve; no personal drafts / dual submit / draftStale. **Staff soft-unpublish/republish** (`inCatalog`; keep live; ≠ block; pack unpublish confirm `content.unpublishConfirm` warns open moderation requests will be cancelled — SC-PACK-139; server cascade SC-PACK-137…141). Collection: Edit (unpublished creator **or** staff any pack) + trash confirm; soft-unpublished → gray «Снято с публикации», non-navigating for non-staff; published in-catalog non-staff → «Добавить набор заданий» only; **no** row `:to` — row `@click` → live if navigable else editor; icon `@click.stop`; hide Edit if `blocked`. Live: staff Edit (exclusive lock) without collection + unpublish/republish; non-staff verified collectors → add-task-set page (in-catalog only). Creator editor: unified Submit (cards + ≥1 task set); staff editor: direct `staffSavePack` (no Submit). Staff queue: open `pending`|`needs_revision`; single Approve / «Доработать» (`needs-revision`); author Cancel. Cascade yellow / D1′ locks retained on unpublished working copy. HTTP via `client.http`. Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified`; staff `requiresStaff`. Capability: `openspec/specs/content/packs/`.
...
| `/support`                            | `support`                     | create + own tickets (incl. `change_pack` + catalog pack select); `meta.requiresAuth`                                  |
...
| `/content/packs`                      | `content-catalog`             | pack catalog (nav → collection; «На модерации» hidden for staff; link from collection); `meta.requiresAuth`            |
| `/content/collection`                 | `content-collection`          | own collection (Edit unpublished creator / staff; **no** row add-task-set; trash; no row `:to`); `meta.requiresAuth`   |
| `/content/my-moderation`              | `content-my-moderation`       | author «На модерации» list (pending\|needs_revision → Edit; non-staff); `meta.requiresAuth`                            |
| `/content/packs/new`                  | `content-pack-new`            | create → redirect answers; `meta.requiresAuth` (+ verify gate)                                                         |
| `/content/packs/:id`                  | `content-pack`                | live pack (staff Edit + lock; add-task-set beside «Задания»; collect from `inCollection`); `meta.requiresAuth`         |
| `/content/packs/:id/edit`             | `content-pack-edit`           | working copy (creator) or staff live edit + lock session cards↔tasks; unified submit / staff save; `meta.requiresAuth` |
| `/content/packs/:id/add-task-set`     | `content-pack-add-task-set`   | post-publish add-only new task set (SC-PACK-107/108); `meta.requiresAuth`                                              |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (unpublished / staff session + staff-save); `meta.requiresAuth`                                        |
| `/content/packs/:id/moderation`       | `content-pack-moderation`     | author moderation thread; `meta.requiresAuth`                                                                          |
| `/content/staff`                      | `content-staff`               | open queue pending\|needs_revision; `requiresStaff`                                                                    |
| `/content/staff/requests/:id`         | `content-staff-request`       | staff hub Approve / needs-revision; no block UI; `requiresStaff`                                                       |
| `/content/staff/requests/:id/tasks`   | `content-staff-request-tasks` | redirect → request hub (legacy bookmark); `requiresStaff`                                                              |
...
- **`support`** (`stores/support.ts`, setup store) — HTTP create/list/detail/message/close + staff list/take/status + admin users/role; `change_pack` requires `packId` (SC-SUP-28); `loading` / `error` + page `q-banner` pattern.
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / `listMyModeration` / live (`inCollection`, `inCatalog`) / working-copy draft (`/draft` = creator unpublished) / unified `submitPack` / add-task-set / staff lock + `staffSavePack` / `isStaffEditSessionNavigation` (cards↔tasks keep lock) / `unpublishPack` / `republishPack` / `unpublishTaskSet` / `republishTaskSet` / `needsRevisionRequest` / `cancelRequest` / `loadModeration` / `postModerationMessage` / author delete unpublished; open = pending\|needs_revision; cascade yellow helpers + Editor/Tasks `cascade-gap-outline` / question slot chips / add-task-set thread / live set summary + drill-in (SC-PACK-126…133); no personal drafts / stale rebase; `loading` / `error` + page `q-banner` (quiet autosave; `pack_unpublished` / `last_published_task_set`).
...
| `client-locate-change-points` | Where to edit/add (incl. content working copy / add-task-set beside «Задания» / staff lock session + cascade yellow + pack/set soft-unpublish + live drill-in + slot text / authorDisplayName / Tasks back SC-PACK-126…136 / trash/`inCollection` / «На модерации» hidden for staff / support `change_pack`; Vitest harness paths)                                                                                                                                                                                                                                                                                 |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content working copy + add-task-set beside «Задания» + staff Edit lock session + trash/click isolation / author «На модерации» / cascade yellow + pack/set soft-unpublish + live drill-in + author on sets / Tasks back SC-PACK-126…136 / author delete unpublished / staff needs_revision / no block UI)                                                                                                                                                                                                                   |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage / Support create (`change_pack` pack select) + content moderation reply `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection + «Карты» → `content-maps`; MapsList/MapEditor; author «На модерации» (hidden for staff; pack+map); content working-copy editor + add-task-set beside «Задания» + staff Edit lock session + stable tooltip hints; trash/click isolation; cascade yellow + slot chips + add-task-set thread + pack/set soft-unpublish (pack confirm warns open requests cancelled SC-PACK-139) + live drill-in + AddTaskSet chips + taskSetLabelFrom / Tasks `back` (SC-PACK-126…136); staff needs_revision + map type badge; support `change_pack`; no block UI; GamePage HUD / board |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` (`change_pack`+packId) / `content` (packs) / `maps` (UGC maps HTTP; shared my-moderation/staff queue type `map`); meta topic `work-with-stores/content.md` for packs                                                                                                                                                                                                                                                                                                                                                                                                   |
| `work-with-styles`            | Quasar Dark + GET/POST `/api/theme`, header always-button brand-logo ≥60px CSS + theme toggle, muted chrome, board holes + grille overlays (`--grille-anim-ms` 1000) + catapult reveal/broken-hold CSS (`--catapult-anim-ms` 1000) + top presence + seated strip HUD / budgets / end-turn affordance CSS; content `.cascade-gap-outline` (SC-PACK-126)                                                                                                                                                                                                                                                             |
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` (packs); `maps.*` (list/editor/unpublishConfirm/errors); game keys                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `work-with-lobby`             | Live LobbyRoom list, create-with-maxSeats + grilleDensity + catapultDensity modal (12/22/35% seed each; no Play), quiet resubscribe; header Support + «Наборы» → `content-collection` + «Карты» → `content-maps`                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL SC-PACK-100…136 + `ContentMaps` / `maps.paint` SC-MAP + `ContentSoftUnpublish` + `ContentFollowUp4` + SupportChangePack; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                                               |
...
Commands (`npm run lint`, `npm run typecheck`, `npm test`, `quasar dev`, `quasar build`) are run by the **agent** from this package root. Do not wait for user confirmation; fix failures before claiming done.
