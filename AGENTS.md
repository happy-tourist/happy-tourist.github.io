...
- Browse content packs: collection → catalog/create; author «На модерации» (`/content/my-moderation`); collection list Edit + trash (`delete`) + confirm remove (click isolation; hide Edit if `blocked`); live Edit when `inCollection` (hide if `blocked`/foreign open; open author keeps Edit; collect from `inCollection`); answers page («Набор карточек») + nested task-set; dual submit; author delete unpublished pack/task-set (confirm); delete-card confirm by `hasLive` (published vs draft); cascade slot clear without false D1′ lock (no client pre-clear); yellow `cascadeGap*` on task + set while gaps; task-row slots; page subtitle = list `taskSetStatusMarks.*`; D1′/D5′ on open (pending|rejected); three-phase marks (ожидает отправки / на модерации / нужна доработка); statuses + threads; staff queue answers + **tasks-only** + open **rejected** («нужна доработка»; approve-from-rejected OK); same hub layout; no answers approve when tasks-only; nested approve → hub (or queue if tasks-only) (no block UI).
...
| `/content/collection`                 | `content-collection`          | own collection (lobby entry; nav → catalog + «На модерации»; Edit + trash + confirm; no row `:to`; hide Edit if `blocked`); `meta.requiresAuth` |
...
| `/content/packs/:id`                  | `content-pack`                | live pack (`inCollection` Edit; hide if `blocked`/foreign pending; collect from `inCollection`); `meta.requiresAuth`                            |
| `/content/packs/:id/edit`             | `content-pack-edit`           | «Набор карточек» (+ three-phase marks; delete-card confirm by `hasLive`; cascade save; author delete unpublished); `meta.requiresAuth`          |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (D1′; yellow cascade gaps; task-row slots; subtitle=`taskSetStatusMarks.*`; author delete set); `meta.requiresAuth`                     |
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection (confirm remove) / `GET /api/content/my-moderation` (`listMyModeration`) / live (`inCollection` + `pendingAnswersAuthorId`/`pendingTasksAuthorId` = **open** pending\|rejected; patch membership on collect/remove) / draft / `submitAnswers` / `submitTasks` / author delete unpublished pack|task-set / moderation + staff; draft statuses + requestId; `answersDirty` / `tasksDirty` + per-set `needsModeration` + open per type; three-phase marks; `cascadeGapTaskIds` + `taskHasCascadeGap` / set-level yellow while gaps (restore on `loadDraft` when dirty+empty slots); no client pre-clear before cascade save; D1′ + D5′ on open mirrored in UI; staff pending|rejected/`tasksOnly`/`answersActionsAvailable` (approve-from-rejected; `not_approvable`); hub omit moderated sets / post-approve → hub (queue if tasks-only); no block UI; `loading` / `error` + page `q-banner` (quiet autosave).
...
| `client-locate-change-points` | Where to edit/add (incl. content packs cascade/hasLive confirm / yellow gaps / task-row slots / D1′/author delete / live Edit in-collection + trash / `inCollection` / author «На модерации» / staff queue reject-stays + hub `tasksOnly`; Vitest harness paths)                                                                                                                                                                                                                                                                                             |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content cascade/hasLive confirm / yellow gaps / task-row slots / live Edit in-collection + trash/click isolation / author «На модерации» / three-phase marks / author delete unpublished / D1′ / staff reject-stays + approve-from-rejected / hub `tasksOnly` / no block UI)                                                                                                                                                                                          |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации»; content trash + click isolation; live Edit when `inCollection`; cascade/hasLive confirm; yellow gaps; task-row slots; subtitle=`taskSetStatusMarks.*`; three-phase marks; staff queue rejected + approve-from-rejected; tasks-only hub; no block UI; GamePage HUD / board                                                                                                                                                                                                                       |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` / `content` (dual submit + my-moderation / cascadeGap* / D1′/D5′ / `inCollection` + pending author ids / three-phase / staff pending\|rejected + approve-from-rejected / `tasksOnly` / no block UI; meta topic `work-with-stores/content.md`)                                                                                                                                                                                                                                                                     |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `content.*` («Набор карточек» / `deleteCardConfirm*` by hasLive / `myModeration*` / `taskSetStatusMarks.*` three-phase + page subtitle sync (aliases `statusCycle*`) / staff rejected label + nested + errors); game keys                                                                                                                                                                                                                                                                                                                                            |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores incl. `content`; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                                                                                                                                  |
