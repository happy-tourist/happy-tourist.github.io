...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection / `listMyModeration` / live (`inCollection`, `inCatalog`) / working-copy draft (`/draft` = creator unpublished) / unified `submitPack` / add-task-set / staff lock + `staffSavePack` / `isStaffEditSessionNavigation` (cards↔tasks keep lock) / `unpublishPack` / `republishPack` / `needsRevisionRequest` / `cancelRequest` / `loadModeration` / `postModerationMessage` / author delete unpublished; open = pending\|needs_revision; cascade yellow helpers + Editor/Tasks `cascade-gap-outline` / question slot chips / add-task-set thread (SC-PACK-126…128); no personal drafts / stale rebase; `loading` / `error` + page `q-banner` (quiet autosave; `pack_unpublished`).
...
| `client-locate-change-points` | Where to edit/add (incl. content working copy / add-task-set beside «Задания» / staff lock session + cascade yellow + SC-PACK-126…128 / trash/`inCollection` / «На модерации» hidden for staff / support `change_pack`; Vitest harness paths)                                                                                                                                                                                                                                                                                                                                  |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content working copy + add-task-set beside «Задания» + staff Edit lock session + trash/click isolation / author «На модерации» / cascade yellow + SC-PACK-126…128 / author delete unpublished / staff needs_revision / no block UI)                                                                                                                                                                                                                                                     |
...
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации» (hidden for staff); content working-copy editor + add-task-set beside «Задания» + staff Edit lock session + stable tooltip hints; trash/click isolation; cascade yellow + slot chips + add-task-set thread (SC-PACK-126…128); staff needs_revision; support `change_pack`; no block UI; GamePage HUD / board                                                                                                                                                                                                                                  |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` (`change_pack`+packId) / `content` (working copy + `submitPack` + add-task-set + staff lock/save + `isStaffEditSessionNavigation` + cascadeGap* + SC-PACK-126…128 / `inCollection` / no block UI; meta topic `work-with-stores/content.md`)                                                                                                                                                                                                                                                                       |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` («Набор карточек» / `addTaskSet*` / `staffEditSubtitle` / `tasksSaveHint` / `questionNeedsSlot` / `taskSetStatusMarks.*` / `moderationThread` / `slotEmpty` / `deleteCardConfirm*` by hasLive / `myModeration*` / `statuses.needs_revision` / errors); game keys                                                                                                                                                                                                                                                                                   |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL SC-PACK-100…128 + `ContentSoftUnpublish` + `ContentModerationUx` + SupportChangePack; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                                                             |
