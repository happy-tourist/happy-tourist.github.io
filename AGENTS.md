...
- Open Game and view the tourist board with synced seats (pieces appear only in `playing`) and a compact 2×2 tourist chip inside the sticky bottom HUD once own pieces exist (no visible «Мои туристы» caption; chip opens an upward `q-menu` full-size picker); after phase `playing`, on own turn select an unfinished piece from the picker or board and submit a one-step `move` via the game store; landing on center finishes a piece (disappear + finish flag on chip/menu); finishing all four shows a place modal and presence badge while the seat stays (say allowed, no moves, leave without confirm); solo five-minute timer expiry or steps exhaustion (no live task tile) shows a distinct end modal + locks moves (`timeExpired`); presence lives in the sticky bottom `.game-hud` (seated: own+budgets → chip → opponents right; spectator: occupied centered; no top-row / side columns) with dual circular countdowns (outer turn blue/red, inner reconnect warning) around a strip-sized avatar; say bubbles stack above avatars toward the board; seated+online players may send preset say bubbles (`hello` / `luck`) via `sendSay`; underfilled waiting may `sendReady`.
...
- Game board / sticky bottom `.game-hud` presence stay on `GamePage` (not in App).
...
- **Game session** - `stores/game` room attach (mirror `seats` with `touristId` + `pieces[]` (+ `finished`/`trapped`) + `connected` / `reconnectUntil` / `ready` / `finishPlace` / `timeExpired` / `phase` / `maxSeats` / `countdownRemaining` / `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys` / `holdingGrilleKeys` / `sessionId`; private `steps`/`peeks`/`budgetsInfinite` (peeks∞ only)/`peekedThisTurn` (legacy)/`openPeek`/`allJailWarning` from `budgets`/`peekOpen`/`allJailWarning`; `isMyTurn` / `isPlaying` / `isMySeatFinished` / `isMySeatTimeExpired` / `canSendEndTurn` / `sendMove` / `sendRescue` / `sendReturnFromFinish` / `sendPeek` / `sendPeekAnswer` / `sendEndTurn` / `sendReady`; create `{ maxSeats, grilleDensity }`) + `pages/GamePage` tourist board (unfinished pieces + disappear + holes + grille overlays + rescue/return-in-menu) + sticky bottom `.game-hud` presence + dual rings + own counters + end-turn + corner affordances + compact 2×2 chip + upward `q-menu` picker once pieces exist + countdown overlay; leave confirm + match status live in `App.vue` header on Game (when seated ∧ `playing` ∧ `!finishPlace` ∧ `!timeExpired`).
- **Tourist board** - client layout constant on GamePage (start/task/center tiles; holes for `removedTaskKeys` — not landable; piece may stand on hole); grille overlay from `holdingGrilleKeys` (`grille.png` drop/rise **`GRILLE_ANIM_MS = 1000`** / `--grille-anim-ms`, incl. leave-clear rise + chrome grille on chip/menu when trapped); trapped pieces visible, no move/peek; rescue affordance when adj free + steps; return control **only** in picker menu beside flag → ring highlights → `sendReturnFromFinish`; overlay unfinished pieces only after materialize; occupied presence in sticky bottom `.game-hud` with dual rings (no top-row / side columns); finish/ready top-left, say top-right (bubbles above avatar); own steps/peeks + «Завершить ход»; peek eye (non-trapped on live `*`); all-jail warning modal (own seat only); on own turn while `playing` local select/hints + `sendMove` (does not end turn; hints exclude holes); center finish → fade + finish flag on chip/menu + place modal.
...
| `work-with-pages`             | Routes + guards; App header (theme + Game leave/status); Lobby grilleDensity; GamePage sticky `.game-hud` / compact chip+`q-menu` / budgets/peek / holes / grilles / dual end + all-jail |
...
| `work-with-styles`            | Quasar Dark + GET/POST `/api/theme`, header, muted chrome, board holes + grille overlays (`--grille-anim-ms` 1000) + sticky `.game-hud` / chip-wrap / chip / budgets/peek CSS |
| `work-with-localization`      | vue-i18n boot; `game.say` / ready / leave / finish / `touristChipAria` / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille density + rescue/return/all-jail keys |
...
| `work-with-game-board`        | Board + sticky bottom `.game-hud` presence + grille (`GRILLE_ANIM_MS=1000`) + compact chip/`q-menu` + trap/rescue/return-in-menu + budgets/peek + dual rings      |
...
- [`../happy-tourist-server`](../happy-tourist-server) — Colyseus multiplayer server (rooms, auth, HTTP `/rooms/:roomName`). Prefer changing room names, state schema, and move protocol in coordination with the server; this client assumes room type `tourist`, mirrors seats (`touristId` + `pieces` + `finished`/`trapped` / `finishPlace` / `timeExpired`) / `phase` / `maxSeats` / `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys` / `holdingGrilleKeys`, listens private `budgets`/`peekOpen`/`allJailWarning`, renders unfinished pieces only after materialize + holes (not landable) + grille overlays + trap/rescue + return-in-menu chrome + sticky bottom `.game-hud` presence with dual rings + own counters (steps number / peeks ∞ solo) /end-turn + peek chrome + compact chip/`q-menu` picker + place/timer-vs-steps/solo-peeks∞/all-jail modals + local move chrome only in `playing` for eligible seats (move does not end turn; targets exclude holes; trapped locked), and sends via `sendMove` / `sendRescue` / `sendReturnFromFinish` / `sendPeek` / `sendPeekAnswer` / `sendEndTurn` / `sendReady`. Create options `{ maxSeats, grilleDensity }`. Leave + match status live in `App.vue` header on Game (no room-id chrome).
