...
- Browse available tourist rooms in the lobby (live `LobbyRoom` subscribe; leave lobby before enter `tourist`); open Support or content packs («Наборы» → collection first) from the lobby header.
- Create support tickets (topic + body), view own list / thread; staff (moderator|admin) queue; admin users page to change roles.
- Browse content packs: collection → catalog/create; author «На модерации» (`/content/my-moderation`); collection list Edit + trash (`delete`) + confirm remove (click isolation; hide Edit if `blocked`; staff unpublish/republish; hide Edit for non-staff after staff-unpublish); live Edit when `inCollection` (hide if `blocked`/foreign open; open author keeps Edit; collect from `inCollection`); answers page («Набор карточек») + nested task-set; dual submit; author delete unpublished pack/task-set (confirm; not while lastLive staff-unpublish); delete-card confirm by `hasLive`; cascade outline (`cascade-gap-outline`, not row `bg-warning`); stale-draft banner + pull; live/staff slot chips; «Опубликовано»/«Одобрено»; staff task-set unpublish when ≥2 live sets; D1′/D5′; three-phase marks; staff queue answers + **tasks-only** + open **rejected**; no block UI.
...
- Shared `q-header` in `App.vue` (always-button brand logo ≥60px left on all pages; theme toggle; on Game also centered match status; leave = logo click). Guest preference → `localStorage` (`ht-theme`); registered → `GET /api/theme` restore (not JWT-only) + `POST /api/theme` on toggle. Tourist board scoped CSS is independent of chrome Dark.
...
- **Content packs** (`content/packs`) - `stores/content` + collection-first nav («Наборы») → catalog/create + author «На модерации» (`content-my-moderation` ← `GET /api/content/my-moderation`). Collection: Edit icon + trash (`delete`) + confirm remove; **no** row `:to` (Quasar race) — row `@click` → live if `hasLive` else editor; icon clicks MUST `@click.stop`; hide Edit if `blocked`; staff unpublish/republish next to Edit; hide Edit for non-staff after staff-unpublish (`unpublishedByStaff`). Live pack: Edit when `inCollection` and (no open request or I am open author); hide Edit if not in collection, `blocked`, or foreign open; collect button from live `inCollection` (not ephemeral `added`). Live GET exposes `inCollection` + `pendingAnswersAuthorId` / `pendingTasksAuthorId` (**open** = pending\|rejected). Answers editor (`…/edit`, «Набор карточек») + nested task-set (`…/tasks/:taskSetId`); dual submit; quiet autosave; author **delete unpublished** pack / task-set (creator only, confirm; not while lastLive staff-unpublish). Staff may unpublish a live task-set when ≥2 live sets remain. Delete-card confirm: published copy iff `hasLive`, else draft (`content.deleteCardConfirmPublished` / `deleteCardConfirm`). Cascade: prefer server slot clear — do **not** pre-clear client slots before save (avoids false `answers_dirty`); after cascade, `cascade-gap-outline` on task + task-set while `cascadeGapTaskIds` gaps remain (not row `bg-warning`); task list rows show answer slots; page subtitles reuse the same `content.taskSetStatusMarks.*` vocabulary as list marks (D48; incl. «Опубликовано»/«Одобрено»; `statusCycle*` kept as aliases). Stale draft: `draftStale` banner + pull/`rebaseDraft`. Live/staff task slots as chips. Draft statuses + requestId; threads; per-set `needsModeration`; `answersDirty` / `tasksDirty`; **three-phase marks** UI: ожидает отправки / на модерации / нужна доработка. **D1′ (D37):** dirty answers without open → lock tasks for all; open (pending\|rejected) author A MAY create/edit tasks while dirty; others blocked. Cascade-only slot clears from card content/delete are **not** a tasks edit under that lock. D5′: while answers open under A, only A submits tasks; open-author resubmit (rejected→pending). Staff queue lists answers-open **and** tasks-only **and** open **rejected** (label «нужна доработка»; approve-from-rejected without mandatory resubmit; cancel drops from queue). Same hub layout (answers context → task-set list); `tasksOnly` / `answersActionsAvailable === false` → hide answers approve/reject; nested tasks → redirect hub after approve, or **queue** when tasks-only (omit moderated sets; no `pack_not_public` dead-end; **no block/unblock UI**). HTTP via `client.http`. Catalog/collection: any JWT; create/edit/submit: non-anonymous + `emailVerified`; staff `requiresStaff`; approve order tasks → answers → catalog. Capability: `openspec/specs/content/packs/`.
...
| `/content/packs`                      | `content-catalog`             | pack catalog (nav → collection + «На модерации»; link from collection); `meta.requiresAuth`                                                     |
| `/content/collection`                 | `content-collection`          | own collection (Edit + trash; staff unpublish/republish; hide Edit if `blocked` or non-staff after staff-unpublish; no row `:to`); `meta.requiresAuth` |
| `/content/my-moderation`              | `content-my-moderation`       | author «На модерации» list (pending\|rejected → Edit); `meta.requiresAuth`                                                                      |
| `/content/packs/new`                  | `content-pack-new`            | create → redirect answers; `meta.requiresAuth` (+ verify gate)                                                                                  |
| `/content/packs/:id`                  | `content-pack`                | live pack (`inCollection` Edit; slot chips; hide if `blocked`/foreign pending; collect from `inCollection`); `meta.requiresAuth`                 |
| `/content/packs/:id/edit`             | `content-pack-edit`           | «Набор карточек» (+ three-phase; cascade outline; draftStale pull; staff task-set unpublish; author delete); `meta.requiresAuth`                |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (D1′; cascade outline; slots; subtitle=`taskSetStatusMarks.*`; draftStale; author delete set); `meta.requiresAuth`              |
| `/content/packs/:id/moderation`       | `content-pack-moderation`     | author moderation thread (type-scoped; also embedded on edit/tasks); `meta.requiresAuth`                                                        |
| `/content/staff`                      | `content-staff`               | answers + tasks-only + rejected («нужна доработка»; approve-from-rejected) queue; `requiresStaff`                                               |
| `/content/staff/requests/:id`         | `content-staff-request`       | staff hub same layout; hide answers actions when tasks-only; no block UI; `requiresStaff`                                                       |
| `/content/staff/requests/:id/tasks`   | `content-staff-request-tasks` | nested tasks → hub after approve (queue if tasks-only); `requiresStaff`                                                                         |
...
- **`content`** (`stores/content.ts`, setup store) — HTTP catalog / collection (confirm remove; staff `unpublishPack`/`republishPack`) / `GET /api/content/my-moderation` (`listMyModeration`) / live (`inCollection` + `unpublishedByStaff`/`hasLastLive` + `pendingAnswersAuthorId`/`pendingTasksAuthorId` = **open** pending\|rejected; patch membership on collect/remove) / draft (`draftStale` + `rebaseDraft`) / `submitAnswers` / `submitTasks` / `unpublishLiveTaskSet` / author delete unpublished pack|task-set (forbid while lastLive staff-unpublish) / moderation + staff; draft statuses + requestId; `answersDirty` / `tasksDirty` + per-set `needsModeration` + open per type; three-phase + `published`/`approved` marks; `cascadeGapTaskIds` + `taskHasCascadeGap` / set-level `cascade-gap-outline` while gaps (restore on `loadDraft` when dirty+empty slots); no client pre-clear before cascade save; D1′ + D5′ on open mirrored in UI; staff pending|rejected/`tasksOnly`/`answersActionsAvailable` (approve-from-rejected; `not_approvable`); hub omit moderated sets / post-approve → hub (queue if tasks-only); no block UI; `loading` / `error` + page `q-banner` (quiet autosave).
...
| `client-locate-change-points` | Where to edit/add (incl. content packs cascade outline / slot chips / draftStale+rebase / staff unpublish/republish / D1′/author delete / live Edit in-collection + trash / `inCollection` / author «На модерации» / staff queue reject-stays + hub `tasksOnly`; Vitest harness paths)                                                                                                                                                                                                                                                                                             |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content cascade outline / slot chips / draftStale / staff unpublish/republish / live Edit in-collection + trash/click isolation / author «На модерации» / three-phase marks / author delete unpublished / D1′ / staff reject-stays + approve-from-rejected / hub `tasksOnly` / no block UI)                                                                                                                                                                                          |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage `q-form` / rules                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации»; content trash + click isolation; staff unpublish/republish; live Edit when `inCollection`; cascade outline; slot chips; draftStale pull; subtitle=`taskSetStatusMarks.*` (published/approved); three-phase marks; staff queue rejected + approve-from-rejected; tasks-only hub; no block UI; GamePage HUD / board                                                                                                                                                                                                               |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` / `content` (dual submit + my-moderation / cascadeGap* / draftStale+rebase / unpublish/republish / D1′/D5′ / `inCollection` + pending author ids / three-phase / staff pending\|rejected + approve-from-rejected / `tasksOnly` / no block UI; meta topic `work-with-stores/content.md`)                                                                                                                                                                                                                                                                    |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `content.*` («Набор карточек» / `deleteCardConfirm*` by hasLive / `myModeration*` / `taskSetStatusMarks.*` three-phase + published/approved + `unpublishedByStaff` / `draftStale*` / `unpublish*` / `republish*` / staff rejected label + nested + errors); game keys                                                                                                                                                                                                                                                                                                           |
| `work-with-lobby`             | Live LobbyRoom list, create-with-maxSeats + grilleDensity + catapultDensity modal (12/22/35% seed each; no Play), quiet resubscribe; header Support + «Наборы» → `content-collection`                                                                                                                                                                                                                                                                                                                                                                        |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. `content` publish UX; lib `__tests__`; topic files in meta skill)                                                                                                                                                                                                                                                                                                                                                                                                  |
