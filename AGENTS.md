...
| `/content/collection`                 | `content-collection`          | own collection (Edit unpublished creator / staff; add-task-set on published; trash; no row `:to`); `meta.requiresAuth`                                 |
| `/content/my-moderation`              | `content-my-moderation`       | author «На модерации» list (pending\|needs_revision → Edit); `meta.requiresAuth`                                                                       |
| `/content/packs/new`                  | `content-pack-new`            | create → redirect answers; `meta.requiresAuth` (+ verify gate)                                                                                         |
| `/content/packs/:id`                  | `content-pack`                | live pack (staff Edit + lock; non-staff add-task-set; collect from `inCollection`); `meta.requiresAuth`                                               |
| `/content/packs/:id/edit`             | `content-pack-edit`           | working copy (creator) or staff live edit + lock; unified submit / staff save; `meta.requiresAuth`                                                     |
...
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks`          | nested task-set (unpublished / staff); `meta.requiresAuth`                                                                                             |
...
| `client-locate-change-points` | Where to edit/add (incl. content working copy / add-task-set / staff lock + cascade yellow / trash/`inCollection` / author «На модерации» / support `change_pack`; Vitest harness paths) |
...
| `client-work-with-structure`  | pages / components / boot / stores / assets placement (always-button brand logo ≥60px; content working copy + add-task-set + staff Edit lock + trash/click isolation / author «На модерации» / cascade yellow / author delete unpublished / staff needs_revision / no block UI) |
| `work-with-forms`             | LoginPage / ForgotPasswordPage / ResetPasswordPage / Support create (`change_pack` pack select) `q-form` / rules |
| `work-with-pages`             | Routes + guards; Lobby «Наборы» → collection; author «На модерации»; content working-copy editor + add-task-set + staff Edit lock; trash/click isolation; cascade yellow; staff needs_revision; support `change_pack`; no block UI; GamePage HUD / board |
| `work-with-stores`            | Pinia `auth` / `theme` / `game` / `support` (`change_pack`+packId) / `content` (working copy + `submitPack` + add-task-set + staff lock/save + cascadeGap* + needs_revision / `inCollection` / no block UI; meta topic `work-with-stores/content.md`) |
...
| `work-with-localization`      | vue-i18n; auth-email RU; `support.*` incl. `change_pack`; `content.*` («Набор карточек» / `addTaskSet*` / `deleteCardConfirm*` by hasLive / `myModeration*` / `statuses.needs_revision` / errors); game keys |
...
| `work-with-test`              | Vitest + `@vue/test-utils` (harness: `vitest.config.ts` / `test/setup.ts` / `npm test`; stores/pages incl. content simplify ACL + SupportChangePack; lib `__tests__`; topic files in meta skill) |
